...
def _panel_resumen_captura(
    ...
    (INDUSTRIA) separando Presupuesto vs Proyectos."""
    df = work_df.copy()
    ...
    c3.metric("total kilos", f"{float(df['_kg'].sum()):,.0f}")
    c4.metric("total USD", f"{float(df['_usd'].sum()):,.2f}")
...
def _panel_pivot(id_carga: int) -> None:
    ...
    for tab_ui, (label, seccion, region) in zip(sub_tabs, _TABS_PIVOT):
        with tab_ui:
            ...
            grid_nonce_key = f"pc_gridnonce_{seccion}_{region}_{id_carga}"
            ...
            grid_response = AgGrid(
                ...
                key=f"pc_pivot_{seccion}_{region}_{id_carga}_{st.session_state[ver_key]}_{st.session_state[grid_nonce_key]}",
            )
