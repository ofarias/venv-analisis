...
def pantalla_documentos_ada():
    ...
    with tab_docs:
        ...
        with col_f1:
            f_desde = st.date_input("desde", value=fdef_desde, format="YYYY-MM-DD", key="docs_desde")
        with col_f2:
            f_hasta = st.date_input("hasta", value=fdef_hasta, format="YYYY-MM-DD", key="docs_hasta")
        with col_rfc:
            rfc = st.text_input("rfc emisor", value="", key="docs_rfc_emisor")
        with col_nom:
            nombre = st.text_input("nombre emisor", value="", key="docs_nombre_emisor")
        with col_folio:
            folio = st.text_input("folio", value="", key="docs_folio")
        with col_rfc_rec:
            rfc_rec = st.text_input("rfc receptor", value="BIO870307QD0", key="docs_rfc_receptor")
        ...
        with col_tipo:
            tipo = st.selectbox("tipo", tipos, index=idx_tipo, key="docs_tipo")
        ...
        with c1:
            page_size_opt = st.selectbox("tamaño página", [200, 500, 1000, "Todas"], index=3, key="docs_page_size")
        with c2:
            page = st.number_input("página", min_value=1, value=1, step=1, disabled=(page_size_opt == "Todas"), key="docs_page")
        with c3:
        ...
        # valores numéricos/fecha ada para match
        ...
        # match 2a: cve_prov + (no_factura/docto) vs (serie+folio o folio)
        ...
        if st.button("insertar seleccionados en sae", key="btn_insertar_masivo"):
            ...
            (st.success if fail == 0 else st.warning)(f"insertados: {ok} | fallidos: {fail}")
                ...
                if docs_df.empty:
                    ...
                    if not all_conceptos:
                        ...
                        st.download_button(
                            ...
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                            use_container_width=True,
                            key="docs_dl_facturas_conceptos",
                        )
        ...
        if not df_cmp.empty:
            ...
            sel_doc = st.selectbox("selecciona un documento", options=lista_docs, key="docs_sel_doc_conceptos")
    ...
    with tab_patrones:
        hoy = date.today()
        f_fin = hoy
        f_ini = (hoy.replace(day=1) - timedelta(days=365))
        ...
        if df_raw.empty:
            ...
            st.warning("no se detectaron patrones que cumplan el mínimo de meses.")
    ...
    with tab_vista:
        ...
        col1, col2 = st.columns(2)
        with col1:
            f_ini_vista = st.date_input("desde", value=date.today().replace(day=1), format="YYYY-MM-DD", key="vista_desde")
        with col2:
            f_fin_vista = st.date_input("hasta", value=date.today(), format="YYYY-MM-DD", key="vista_hasta")
    ...
    with tab_detalles:
        ...
        with col1:
            proveedor = st.text_input("Proveedor (nombre o RFC)", key="det_proveedor")
        with col2:
        ...
        with col3:
            anio_sel = st.number_input("Año", min_value=2020, max_value=anio_actual + 1, value=anio_actual, step=1, key="det_anio_sel")
        ...
        if df_conceptos.empty:
            ...
            def to_float(series):
                return (
                    series.replace({"[$,]": ""}, regex=True).astype(float).sum()
...
def grafica_montos_por_dia(df: pd.DataFrame):
    ...
    for rect, val in zip(barras, serie.values):
        ax.text(rect.get_x() + rect.get_width() / 2.0, rect.get_height() + (0.01 * (serie.max() if serie.max() else 1)),
...
def grafica_detalle_tabla(detalle: pd.DataFrame, top_prov: int = 10, top_conc: int = 8):
    ...
    top_proveedores = (
        det.groupby("NOMBRE_PROV", as_index=False)["USOS"].sum()
    ...
    top_conceptos = (
        det.groupby("DESCR", as_index=False)["USOS"].sum()
        .sort_values("USOS", ascending=False)
        .head(int(top_conc))
    )["DESCR"].tolist()
