...
def mostrar_documentos():
    ...
    if "opciones_busqueda_docs" not in st.session_state:
        st.session_state["opciones_busqueda_docs"] = obtener_documentos_accesibles_resumen(username, roles_usuario)

    # --- Filtros ---
    with st.expander("🔍 Filtros test"):
        opciones_docs = {"Todos": None}
        for f in st.session_state["opciones_busqueda_docs"]:
            etiqueta = f["titulo"]
            if f.get("nombre_archivo") and f["nombre_archivo"] != f["titulo"]:
                etiqueta = f"{f['titulo']} — {f['nombre_archivo']}"
            opciones_docs[etiqueta] = f["id"]

        doc_seleccionado = st.selectbox("Buscar por título", list(opciones_docs.keys()))
        ...
        if st.button("🔎 Buscar documentos"):
            st.session_state["doc_id_busqueda"] = opciones_docs[doc_seleccionado]
        ...
        if st.button("🧹 Limpiar filtros"):
            for key in ["filtros_aplicados", "tipo_filtrado", "fecha_inicio", "fecha_fin", "doc_id_busqueda"]:
    ...
    if st.session_state.get("filtros_aplicados"):
        ...
        doc_id_filtro = st.session_state.get("doc_id_busqueda")
    ...
    if selected is not None and not selected.empty:
        ...
        if doc:
            ...
            puede_editar = "Editar" in doc["permisos"]
